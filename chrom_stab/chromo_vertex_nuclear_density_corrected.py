""" 
F_BIND CORRECTED- fixed so that has explicit cross interaction terms. 

can have up to 4 reader proteins (2 of each type) bound to a given nucleosome
...
def eval_f_bind_cooperative_arr(n_b_arr, n_m_arr_1, n_m_arr_2, e_m_arr,\
    ...
    for i,n_m_pair in enumerate(n_m_pair_arr):
        ...
        f_bind = eval_f_bind_cooperative(n_b_arr, n_m_pair, e_m_arr,j_int_g1g1, j_int_g1g2, j_int_g2g2)
        
        f_bind_arr[i] = f_bind
    return f_bind_arr

def calc_saddle_point_E(f_bars, rho_c, v_int, mu, poly_marks, e_m, f_binds): 
    ...
    # TECHNICALLY divided by M number of nuclesomes. since used to compare free enrgies, its fine
    # calc mean-field protein-protein interaction
    mf_pp_int = 0#0.5 * rho_c * M # 0.5*phi_c**2
    for g1 in range(len(f_bars)):
        for g2 in range(len(f_bars)):
            mf_pp_int += Vol_int*v_int[g1][g2]*f_bars[g1]*f_bars[g2]
    mf_pp_int *= 0.5 * rho_c
    
    # calc binding partition function
    phi_bind_arr = rho_c * np.array(f_bars)
...
#                 erg_ints[i] = np.squeeze(erg_ints[i])
            ...
            for om_1 in range(3): 
                ...
                ## 00, 01, 10, 11, 20, 02
                    ...
                    sg2_avg_raw = 0*P00 + 0*P10 + 1*P01 + 0*P20 + 2*P02 + 1*P11 + 1*P21+ 2*P12+ 2*P22
