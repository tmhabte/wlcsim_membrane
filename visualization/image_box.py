...
def gen_pymol_file(r_poly, meth_seq = np.array([]), hp1_seq = np.array([]), limit_n = False, n_max = 100000,
                   ...
                   box_adjust_l = np.array([]), box_adjust_r = np.array([]) ):
    r"""
    ...
    resnum = 1
    numresidues = len(r_poly[:, 0]) #40000
...
def find_ind_mid_slice(r_poly, n_max, ring, box_adjust_l, box_adjust_r):
    r"""
...
def find_connect(ind_save, ring, box_adjust_l, box_adjust_r):
    r"""
    ...
    for ind in range(len(ind_save)):
        if ind_save[ind] * ind_save[ind - 1] * box_adjust_l[ind] == 1:
            connect_left[ind] = 1

        if ind == (len(ind_save) - 1):
            if ind_save[ind] * ind_save[0] * box_adjust_r[ind] == 1:
                connect_right[ind] = 1
        else:
            if ind_save[ind] * ind_save[ind + 1] * box_adjust_r[ind] == 1:
                connect_right[ind] = 1
