...
mu_max_1 = -2.9#8#0.1 #10
mu_min_1 = -3#-9
del_mu_1 = .1#.5 #0.25

mu_max_2 = -2.9#8#0.1 #10
mu_min_2 = -3#-9
del_mu_2 = .1#.5 #0.25
...
# 
chi_AB = 25 / (phi_p*N) #(v_int[0,1] - 0.5*(v_int[0,0] + v_int[1,1]))*Vol_int/100
chi_AS = 00 / N #834 / N
chis = [chi_AB, chi_AS]
...
np.save("OABS_phases_arr_zoomed_chiABphipNeq"+str(int(chi_AB*phi_p*N))+"N="+str(int(N)), phases)
np.save("OABS_min_F_arr_zoomed_chiABphipNeq"+str(int(chi_AB*phi_p*N)), minF_arr)
