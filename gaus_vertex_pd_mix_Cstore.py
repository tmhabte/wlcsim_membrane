...
class C_store:
    def __init__(self):
        #need to match M_max,ks 
        self.C2s = {}
        self.C3s = {}
        self.C4s = {}
        
    def add_C2(self, mix, Ks, C):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        if key not in self.C2s.keys():
            self.C2s[key] = C
        else:
            raise Exception("ALready have this key. WHY ADDING TWICE?")
    
    def pull_C2(self, mix, Ks):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        if key in self.C2s.keys():
            return self.C2s[key]
    
    def have_C2(self, mix, Ks):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        return key in self.C2s.keys()
    
    def add_C3(self, mix, Ks, C):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        if key not in self.C3s.keys():
            self.C3s[key] = C
        else:
            raise Exception("ALready have this key. WHY ADDING TWICE?")
    
    def pull_C3(self, mix, Ks):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        if key in self.C3s.keys():
            return self.C3s[key]
    
    def have_C3(self, mix, Ks):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        return key in self.C3s.keys()
    
    def add_C4(self, mix, Ks, C):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        if key not in self.C4s.keys():
            self.C4s[key] = C
        else:
            raise Exception("ALready have this key. WHY ADDING TWICE?")
    
    def pull_C4(self, mix, Ks):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        if key in self.C4s.keys():
            return self.C4s[key]
    
    def have_C4(self, mix, Ks):
        M_max = mix.M_max
        key = str(Ks) + str(M_max)
        return key in self.C4s.keys()
...
def calc_sf2(mix, c_stor, k_vec = np.logspace(-2, 2, 50), plotting = False):
    N = mix.N
    M = mix.M_ave
    M_max = mix.M_max
    N_m = mix.N_m
    ...
    for i, k in enumerate(k_vec):
        if not plotting and c_stor.have_C2(mix, k_vec):
            C = c_stor.pull_C2(mix, k_vec)
        else:
...
def calc_sf2_inv(mix, c_stor, k_vec = np.logspace(-2, 2, 50)):
    N = mix.N
    M = mix.M_max
    N_m = mix.N_m
    b = mix.b
    ...
    (S2_AA_arr, S2_AB_arr, S2_BA_arr, S2_BB_arr) = calc_sf2(mix, c_stor, k_vec)
    det = S2_AA_arr * S2_BB_arr - (S2_AB_arr*S2_BA_arr)
    S2_AA_inv = S2_BB_arr * (1/det)
    S2_AB_inv = -S2_AB_arr * (1/det)
    S2_BA_inv = -S2_BA_arr * (1/det)
...
def calc_sf3(mix, c_stor, k_vec, k_vec_2, plotting = False):
    ...
    for i, k_1 in enumerate(k_vec):
        if not plotting and c_stor.have_C3(mix, Ks):
            C = c_stor.pull_C3(mix, Ks)
        else:
            k_2 = k_vec_2[i]
            k_12 = k_1 + k_2
...
def calc_sf4(mix, c_stor, k_vec, k_vec_2, k_vec_3, plotting = False):
    Ks = [k_vec, k_vec_2, k_vec_3]
    N = mix.N
    M = mix.M_ave
    M_max = mix.M_max
    N_m = mix.N_m
    ...
    for i, k1 in enumerate(k_vec):
        if not plotting and c_stor.have_C4(mix, Ks):
            C = c_stor.pull_C4(mix, Ks)
#             print("using saved sf4!!")
        else:
            k2 = k_vec_2[i]
            k3 = k_vec_3[i]
            k12 = k1 + k2
...
def gamma2_E(mix, c_stor, k, chi):
# like an ensemble-averaged G2, using gaussian chain 
    s2_inv = calc_sf2_inv(mix, c_stor, [k])
    N = mix.N
...
#         raise Exception('Qs must have same length')
    ...
    s2inv = calc_sf2_inv(mix, c_stor, [K1])
    s2inv_2 = calc_sf2_inv(mix, c_stor, [K2])
    s2inv_3 = calc_sf2_inv(mix, c_stor, [K3])
    
    s3 = calc_sf3(mix, c_stor, [K1], [K2])
...
def gamma4_E(mix, c_stor, Ks):
    ...
    s4 = calc_sf4(mix, c_stor, [K1], [K2], [K3]) 
    s31 = calc_sf3(mix, c_stor, [K1], [K2])
    s32 = calc_sf3(mix, c_stor, [K1], [K3])
    s33 = calc_sf3(mix, c_stor, [K1], [K4])
    s2inv = calc_sf2_inv(mix, c_stor, [K])
    s21inv = calc_sf2_inv(mix, c_stor, [K12])
    s22inv = calc_sf2_inv(mix, c_stor, [K13])
    s23inv = calc_sf2_inv(mix, c_stor, [K14])

    s2inv_2 = calc_sf2_inv(mix, c_stor, [K2])
    s2inv_3 = calc_sf2_inv(mix, c_stor, [K3])
    s2inv_4 = calc_sf2_inv(mix, c_stor, [K4])
